...
gpu_bsw_driver::kernel_driver_dna(std::vector<std::string> reads, std::vector<std::string> contigs, gpu_bsw_driver::alignment_results *alignments, short scores[4])
{
    ...
    // hipDeviceProp_t prop[deviceCount];
    // for(int i = 0; i < deviceCount; i++)
    //   hipGetDeviceProperties(&prop[i], 0);
    ...
    { 
      ...
      int my_cpu_id = 0;//omp_get_thread_num();
      hipSetDevice(1);
      int myGPUid;
      hipGetDevice(&myGPUid);
      float total_time_cpu = 0;
      hipStream_t streams_cuda[NSTREAMS];
      for(int stm = 0; stm < NSTREAMS; stm++){
        hipStreamCreate(&streams_cuda[stm]);
      }
       ...
       // if(my_cpu_id == 0)std::cout<<"Number of GPUs being used:"<<omp_get_num_threads()<<"\n";
        ...
        hipHostMalloc(&offsetA_h, sizeof(int)*(stringsPerIt + leftOvers));
        unsigned* offsetB_h;// = new unsigned[stringsPerIt + leftOvers];
        hipHostMalloc(&offsetB_h, sizeof(int)*(stringsPerIt + leftOvers));

        char *strA_d, *strB_d;
        CHECK(hipMalloc(&strA_d, maxContigSize * (stringsPerIt + leftOvers) * sizeof(char)));
        CHECK(hipMalloc(&strB_d, maxReadSize *(stringsPerIt + leftOvers)* sizeof(char)));

        char* strA;
        hipHostMalloc(&strA, sizeof(char)*maxContigSize * (stringsPerIt + leftOvers));
        char* strB;
        hipHostMalloc(&strB, sizeof(char)* maxReadSize *(stringsPerIt + leftOvers));
        ...
        for(int perGPUIts = 0; perGPUIts < its; perGPUIts++)
        {
            ...
            hipLaunchKernelGGL(gpu_bsw::sequence_dna_kernel, dim3(sequences_per_stream), dim3(minSize), ShmemBytes, streams_cuda[0], 
            ...
            hipLaunchKernelGGL(gpu_bsw::sequence_dna_kernel, dim3(sequences_per_stream + sequences_stream_leftover), dim3(minSize), ShmemBytes, streams_cuda[1], 
            ...
            hipStreamSynchronize (streams_cuda[0]);
            hipStreamSynchronize (streams_cuda[1]);
            ...
            hipLaunchKernelGGL(gpu_bsw::sequence_dna_reverse, dim3(sequences_per_stream), dim3(newMin), ShmemBytes, streams_cuda[0], 
                    strA_d, strB_d, gpu_data.offset_ref_gpu, gpu_data.offset_query_gpu, gpu_data.ref_start_gpu,
                    gpu_data.ref_end_gpu, gpu_data.query_start_gpu, gpu_data.query_end_gpu, gpu_data.scores_gpu, matchScore, misMatchScore, startGap, extendGap);
            CHECK(hipGetLastError());

            hipLaunchKernelGGL(gpu_bsw::sequence_dna_reverse, dim3(sequences_per_stream + sequences_stream_leftover), dim3(newMin), ShmemBytes, streams_cuda[1], 
                    strA_d + half_length_A, strB_d + half_length_B, gpu_data.offset_ref_gpu + sequences_per_stream, gpu_data.offset_query_gpu + sequences_per_stream ,
                    gpu_data.ref_start_gpu + sequences_per_stream, gpu_data.ref_end_gpu + sequences_per_stream, gpu_data.query_start_gpu + sequences_per_stream, gpu_data.query_end_gpu + sequences_per_stream,
                    gpu_data.scores_gpu + sequences_per_stream, matchScore, misMatchScore, startGap, extendGap);
            CHECK(hipGetLastError());
        ...
        CHECK(hipFree(strA_d));
        CHECK(hipFree(strB_d));
        hipHostFree(offsetA_h);
        hipHostFree(offsetB_h);
        hipHostFree(strA);
        hipHostFree(strB);

        for(int i = 0; i < NSTREAMS; i++)
          hipStreamDestroy(streams_cuda[i]);
  ...
  //  #pragma omp parallel
    {
     ...
     // if(my_cpu_id == 0)std::cout<<"Number of GPUs being used:"<<omp_get_num_threads()<<"\n";
      ...
      CHECK(hipMalloc(&d_encoding_matrix, ENCOD_MAT_SIZE * sizeof(short)));
      CHECK(hipMalloc(&d_scoring_matrix, SCORE_MAT_SIZE * sizeof(short)));
      CHECK(hipMemcpy(d_encoding_matrix, encoding_matrix, ENCOD_MAT_SIZE * sizeof(short), hipMemcpyHostToDevice));
      CHECK(hipMemcpy(d_scoring_matrix, scoring_matrix, SCORE_MAT_SIZE * sizeof(short), hipMemcpyHostToDevice));
      ...
      for(int perGPUIts = 0; perGPUIts < its; perGPUIts++)
      {
          ...
          hipLaunchKernelGGL(gpu_bsw::sequence_aa_kernel, dim3(sequences_per_stream), dim3(minSize), ShmemBytes, streams_cuda[0], 
              strA_d, strB_d, gpu_data.offset_ref_gpu, gpu_data.offset_query_gpu, gpu_data.ref_start_gpu,
          ...
          hipLaunchKernelGGL(gpu_bsw::sequence_aa_kernel, dim3(sequences_per_stream + sequences_stream_leftover), dim3(minSize), ShmemBytes, streams_cuda[1], 
              strA_d + half_length_A, strB_d + half_length_B, gpu_data.offset_ref_gpu + sequences_per_stream, gpu_data.offset_query_gpu + sequences_per_stream,
                gpu_data.ref_start_gpu + sequences_per_stream, gpu_data.ref_end_gpu + sequences_per_stream, gpu_data.query_start_gpu + sequences_per_stream, gpu_data.query_end_gpu + sequences_per_stream,
          ...
          hipLaunchKernelGGL(gpu_bsw::sequence_aa_reverse, dim3(sequences_per_stream), dim3(newMin), ShmemBytes, streams_cuda[0], 
                  strA_d, strB_d, gpu_data.offset_ref_gpu, gpu_data.offset_query_gpu, gpu_data.ref_start_gpu,
          ...
          hipLaunchKernelGGL(gpu_bsw::sequence_aa_reverse, dim3(sequences_per_stream + sequences_stream_leftover), dim3(newMin), ShmemBytes, streams_cuda[1], 
                  strA_d + half_length_A, strB_d + half_length_B, gpu_data.offset_ref_gpu + sequences_per_stream, gpu_data.offset_query_gpu + sequences_per_stream ,
                  gpu_data.ref_start_gpu + sequences_per_stream, gpu_data.ref_end_gpu + sequences_per_stream, gpu_data.query_start_gpu + sequences_per_stream, gpu_data.query_end_gpu + sequences_per_stream,
                  gpu_data.scores_gpu + sequences_per_stream, openGap, extendGap, d_scoring_matrix, d_encoding_matrix);
          CHECK(hipGetLastError());
